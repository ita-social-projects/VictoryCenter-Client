...
## Phase 1: Analysis (think through all issues before making any edits)
...
### 1.2 Read changed files in full

Read every changed file completely — smells hide in context, not just the diff. Group reads by area:
- Page components, hooks, contexts
- API services, types, validation schemas
- Tests, constants, config files

### 1.3 Apply review checklists

While reading files, collect all issues against the five categories below. List them all before making any edits.
...
### 1.4 Categorize issues before fixing

Present the full list of found issues before touching any file:
...
- Never commit `.github/copilot-instructions.md` internal artifacts or `NUL`
