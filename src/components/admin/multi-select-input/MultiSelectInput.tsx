...
export const MultiSelectInput = <T,>({
    ...
    return (
        <div className={styles['multiselect']} ref={multiselectContainerRef}>
            <button
                id={id}
                type="button"
                className={classNames(styles['multiselect__placeholder-container'], {
                    [styles['multiselect__placeholder-container--opened']]: isOpen,
                    [styles['multiselect__placeholder-container--disabled']]: disabled,
                })}
            ...
            >
                <div
                    className={classNames(styles['multiselect__placeholder'], {
                        [styles['multiselect__placeholder--has-value']]: value.length > 0,
                    })}
                >
                    <div className={styles['multiselect__placeholder-content']}>{displayLabel}</div>
                    <div className={styles['multiselect__chevron']}>{isOpen ? <ArrowUp /> : <ArrowDown />}</div>
                </div>
            </button>

            {isOpen && !disabled && (
                <div className={styles['multiselect__options-container']} role="listbox" aria-multiselectable="true">
                    {options.map((option) => {
                        ...
                        return (
                            <div
                                key={optionId}
                                className={classNames(styles['multiselect__option'], {
                                    [styles['multiselect__option--selected']]: selected,
                                })}
                                ...
                                tabIndex={0}
                            >
                                <div className={styles['multiselect__option-checkbox']}>
                                    {selected ? <CheckedBox /> : <UncheckedBox />}
                                </div>
                                <span className={styles['multiselect__option-content']}>{getOptionName(option)}</span>
                            </div>
                        );
                    })}
                </div>
