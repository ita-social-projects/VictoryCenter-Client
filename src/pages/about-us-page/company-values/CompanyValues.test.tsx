...
describe('Company Values Section', () => {
    ...
    it('should contain correct text', () => {
        ...
        expect(
            screen.getByText(/Створюємо спільноту підтримки,.*разом ми сильніші./i)
        ).toBeInTheDocument();

        expect(screen.getByRole('heading', {name: 'Різноманіття'})).toBeInTheDocument();
        expect(
            screen.getByText(/Ми поважаємо та цінуємо унікальний досвід.*з якою взаємодіємо./i)
        ).toBeInTheDocument();

        expect(screen.getByRole('heading', {name: 'Стійкість'})).toBeInTheDocument();
        expect(
            screen.getByText(/Віримо у силу духу людини,.*та зміцнення життєстійкості./i)
        ).toBeInTheDocument();
