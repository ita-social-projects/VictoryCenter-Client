...
describe('getPlainTextFromHtml', () => {
    describe('Basic text extraction', () => {
        test.each([
            ...
            expect(getPlainTextFromHtml(input)).toBe(expected);
        });
    ...
    describe('Text formatting and structure', () => {
        test.each([
    ...
    describe('Special characters and entities', () => {
        test.each([
            ...
            [
                'HTML entities',
                '<p>&quot;Quoted Text&quot; &apos;Single Quote&apos;</p>',
                '"Quoted Text" \'Single Quote\'',
            ],
        ])('should handle %s', (_description, input, expected) => {
            expect(getPlainTextFromHtml(input)).toBe(expected);
        });
    ...
    describe('Element filtering and sanitization', () => {
        it('should handle HTML with self-closing tags', () => {
            expect(getPlainTextFromHtml('<p>Before<img src="image.png">After</p>')).toBe('BeforeAfter');
        });

        it('should handle HTML with comments', () => {
            expect(getPlainTextFromHtml('<p>Visible<!-- hidden comment -->Text</p>')).toBe('VisibleText');
        });

        it('should strip script tags', () => {
            expect(getPlainTextFromHtml('<p>Before<script>console.log("test");</script>After</p>')).toBe('BeforeAfter');
        });

        it('should strip style tags', () => {
            expect(getPlainTextFromHtml('<p>Before<style>.class { margin: 0; }</style>After</p>')).toBe('BeforeAfter');
        });
    });
    ...
    describe('Complex HTML structures', () => {
        test.each([
            ...
            [
                'HTML with nested divs and spans',
                '<div><div><span>Inner</span></div><div>Outer</div></div>',
                'InnerOuter',
            ],
            [
