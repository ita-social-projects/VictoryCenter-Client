...
describe('EVENT_CATEGORY_VALIDATION_FUNCTIONS.validateName', () => {
    const { min, max, getRequiredError, getMinError, getMaxError } =
        EVENT_CATEGORY_VALIDATION.name;

    it('returns undefined for a valid name', () => {
        const value = 'Valid category name';

        expect(
            EVENT_CATEGORY_VALIDATION_FUNCTIONS.validateName(value)
        ).toBeUndefined();
    });
