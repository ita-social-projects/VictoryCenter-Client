...
describe('ProgramCategoryValidationSchema', () => {
    it('should pass validation with valid name', async () => {
        ...
        await expect(ProgramCategoryValidationSchema.validate(validData)).resolves.toBeTruthy();
    });

    it('should fail validation when name is missing', async () => {
        const invalidData = {};

        await expect(ProgramCategoryValidationSchema.validate(invalidData)).rejects.toThrow(
            PROGRAM_CATEGORY_VALIDATION.name.getRequiredError(),
        );
    });
    ...
    it('should handle null input', async () => {
        await expect(ProgramCategoryValidationSchema.validate(null)).rejects.toThrow();
    });

    it('should handle undefined input', async () => {
        await expect(ProgramCategoryValidationSchema.validate(undefined)).rejects.toThrow();
    });

    it('should handle non-object input', async () => {
        await expect(ProgramCategoryValidationSchema.validate('string')).rejects.toThrow();
    });
});
